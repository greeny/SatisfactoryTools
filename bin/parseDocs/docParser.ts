...
export const parseDocs = (inputPath: string, workingDir: string) => {
	const docs = safeParseJson<any>(inputPath, 'utf16le');
	const oldData = safeParseJson<IJsonSchema>(path.join(workingDir, 'data.json'));
	...
	fs.writeFileSync(path.join(workingDir, 'data.json'), JSON.stringify(json, null, '\t') + '\n');

	const diffGenerator = new DiffGenerator();
	fs.writeFileSync(path.join(workingDir, 'diff.txt'), DiffFormatter.diffToMarkdown(diffGenerator.generateDiff(oldData, json)));

	fs.writeFileSync(path.join(workingDir, 'imageMapping.json'), JSON.stringify(imageMapping, null, '\t') + '\n');
};